...
## Session 37: [Make `getChunk` Async](./prompts/SESSION_37_Async_GetChunk.md) (Upcoming)
Make `IChunkManager.getChunk` return `Promise<IChunk>`. On cache miss, `ChunkManager` calls an injected `ChunkPersistenceLoader` to fetch the doc from MongoDB, determine if it is a custom or noise chunk, and build the correct `Chunk` in one step (mines + revealed + flagged). Deletes `applyPersistedChunkState` and `loadCustomChunkConfigs`. All call sites add `await`; test mocks switch to `mockResolvedValue`.
