...
## Session 36: [Custom Chunk Generation](./prompts/SESSION_36_Custom_Chunk_Generation.md) (Upcoming)
ChunkConfig discriminated union (`noise` | `custom`) on the chunk document. Runtime branches on config type when building the in-memory Chunk. Pre-gen tool with edge blending: interior chunks fully custom, edge chunks use authored layout for interior cells and worldGenerator for the outer ring so adjacent noise chunks never see a seam.
