...
*   **Goal:** Switch backend persistence from PostgreSQL to MongoDB for infinite board state.
*   **Changes:**
    ...
    *   Created `db.ts` with MongoDB connection logic, document interfaces, and basic interaction functions (save/load chunks, save/load game).
    *   Updated `docker-compose.yml` to replace the PostgreSQL service with MongoDB, updated environment variables and volumes.
...
## Session 2: Unit Testing PlayerActionService (2025-05-02)

**Goal:** Create comprehensive unit tests for `PlayerActionService.handleRevealTile` function.
...
- Implemented comprehensive test cases:
  ...
  - Edge cases: Already revealed cell, locked out player, expired lockout, non-existent game/player, error handling
- Added code coverage configuration to Jest:
  - Added coverage settings to `jest.config.js`
  - Ran tests with coverage reporting (`--coverage` flag)
  - Verified high coverage for `PlayerActionService` (93% statement coverage, 90% branch coverage)
