...
## Session 36: [Custom Chunk Generation](./prompts/SESSION_36_Custom_Chunk_Generation.md) (2026-05-12)
Added `ChunkConfig` discriminated union (`noise` | `custom`) to `ChunkDocument`. `Chunk` constructor accepts `minesOverride?: Uint8Array`. `ChunkRepository` gains `saveCustomChunk` (used by the pre-gen tool) and `decodeMines`. Rewrote `tools/pregen-chunks.ts` as a 7×7 perfect maze generator (recursive backtracking, seed=42): each chunk is a room with 1-cell mine walls and 3-cell doorways — one click floods the entire 49-room maze. Edge blending overrides only the truly outward-facing ring of section-boundary chunks. Added `make pregen` target. Fixed nginx 502 stale-IP bug: added Docker's internal resolver + variable-based `proxy_pass` so upstream IPs are re-resolved after container restarts.

## Session 37: [Make `getChunk` Async + Bulk Preload + Flood Fill Fix](./prompts/SESSION_37_Async_GetChunk.md) (2026-05-14)
Made `IChunkManager.getChunk` return `Promise<IChunk>` with an injected `ChunkPersistenceLoader` that fetches from MongoDB on cache miss, builds the correct `Chunk` (custom buffer or noise) in one step, and applies revealed/flagged state. Deleted `applyPersistedChunkState`, `loadCustomChunkConfigs`, and the stale `socketServer.ts`. Added `ChunkRepository.loadMany` + `ChunkManager.preloadMany` for single-query bulk preloading in `subscribeToChunks`. Fixed `runGlobalFloodFill` to read mine state from chunk tiles instead of WorldGenerator (so custom chunk overrides are respected). Rewrote custom chunk buffer encoding to `0xFF = mine, 0–8 = adjacentMines` baked in at pregen time; updated `pregen-chunks.ts` to use a two-pass outer-face-preserving layout approach.

## Session 38: [Seed Consistency & "SweepTogether" Text Pregen](./prompts/SESSION_38_Seed_Consistency_Text_Pregen.md) (2026-05-15)
Fixed seed mismatch between the pregen tool and the live server: `GameDocument.seed` changed from a random integer to a string (`gameId`), so both `pregen-chunks.ts`/`pregen-text.ts` and the server always use `WorldGenerator("default")` for the same game. Added `tools/pregen-text.ts`: a stroke-font generator that writes "SweepTogether" as 1-cell-wide Bresenham mine strokes (64×96 cells per letter, S=16 scale) surrounded by open background chunks. Enforces all three custom-chunk seam rules and validates every chunk's noise-facing borders before saving via `validateSeam`. Removed the maze from the default board; text is centred around global (0, 0).
