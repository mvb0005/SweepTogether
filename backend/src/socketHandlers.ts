 ...
 * @param data - The payload containing row and column to reveal
 * @param io - The Socket.IO server instance
 */
export function handleRevealTile(socket: Socket, data: unknown, io: Server): void {
  ...
  if (typeof data !== 'object' || data === null || 
    ...
    emitError(socket, 'Invalid revealTile data format. Expected { row: number, col: number }.');
    return;
  }
 ...
 * @param data - The payload containing row and column to flag
 * @param io - The Socket.IO server instance
 */
export function handleFlagTile(socket: Socket, data: unknown, io: Server): void {
  ...
  if (typeof data !== 'object' || data === null || 
      typeof (data as any).row !== 'number' || 
      typeof (data as any).col !== 'number') {
    emitError(socket, 'Invalid flagTile data format. Expected { row: number, col: number }.');
    return;
  }
  
  const { row, col } = data as FlagTilePayload;
